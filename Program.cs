...
public class Program
{
    static void Main(string[] args)
    {
        ...
        Console.WriteLine("\nReadFile");
        calendar = jsonSerializer.Read<FestivalCalendar>(file);
        calendar.ShowFestivals();
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            int m2 = r.Next(m1, 12);
            int d2 = r.Next(d1, 30);
            calendar.AddFestival(new ComicsFestival(new DateTime(y1, m1, d1), new DateTime(y2, m2, d2), calendar.Festivals[i], characters[r.Next(0, characters.Length - 1)], r.Next(1, 10)));
        }

        file = Path.Combine(path, "raw_data.xml");
        xmlSerializer.Write(calendar, file);

        Festival.Sort(calendar);

        file = Path.Combine(path, "data.xml");
        xmlSerializer.Write(calendar, file);

        Console.WriteLine("\nraw_data.xml");
        file = Path.Combine(path, "raw_data.xml");
        calendar = xmlSerializer.Read<FestivalCalendar>(file);
        calendar.ShowFestivals();
        Console.WriteLine("\ndata.xml");
        file = Path.Combine(path, "data.xml");
        calendar = xmlSerializer.Read<FestivalCalendar>(file);
